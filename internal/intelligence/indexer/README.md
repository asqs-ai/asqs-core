...
Nest HTTP routes are indexed as **`API_ROUTE`** symbols with stable `fq_name` so **`ROUTE_TO_HANDLER`** edges persist in Postgres. The **minimal Java indexer** (`tools/java-indexer`) emits the same for **Spring Web** `@RestController` / mapping annotations. The **advanced Java JAR** (`JavaIndexer.java` + `advanced.go`) adds the same E2E-oriented entities as JS/TS where applicable: **`E2E_SPEC`**, Playwright-Java **`TEST_SELECTOR`** (`getByTestId`), and **`API_CLIENT_REQUEST`** + **`CALLS_API`** (WebClient `.uri`, RestTemplate URL strings). The **JS/TS indexer** adds **`PAGE_ROUTE`** for SPA routers, **`E2E_SPEC`**, HTTP client calls, etc. The **C# Roslyn indexer** (`tools/csharp-indexer`, when **`indexer.csharp.indexer_dll_path`** is set) emits **`API_ROUTE`** for ASP.NET attribute routes, **`API_CLIENT_REQUEST`** for **`HttpClient`** usage, and **`E2E_SPEC`** stubs for Playwright .NET / Selenium test files — same post-run **`TARGETS_API_ROUTE`** linking (`apiclient_route_link.go`). Chunk types `route`, `api_contract`, `e2e_pattern`, `page` are assigned in `chunk.go`.
