...
func auditGapFilter(ctx context.Context, opts PlanOptions, byReason map[string]int, totalCandidates, remaining int) {
	...
	opts.Audit.Log(ctx, "plan.gaps_filtered_ineligible", map[string]interface{}{
		"message":          "Dropped gap candidates that do not represent testable behaviour (interface members with no body, framework @Bean factories, trivial accessors, unexported TypeScript functions, C# internals the test assembly has no grant into).",
		"by_reason":        byReason,
