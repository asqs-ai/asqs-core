...
| **Similar reference chunks** | Per profile, vector **Search** per **`chunk_type`** into a **pool** (larger than the output cap), then **MMR** (Maximal Marginal Relevance, cosine in embedding space; λ=0.5, fixed) to reduce near-duplicate neighbors; listing fallbacks if the pool is empty. **Caps:** built-in defaults of 5 / 15 / 5, overridable per profile through **`retrieval.profile_budgets`** (see **`budgets.go`**). The global caps were frozen into those constants; per-profile budgets are where tuning is meaningful. |
