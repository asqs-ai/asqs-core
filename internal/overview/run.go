...
func (g *LLMOverviewDocGenerator) Generate(ctx context.Context, meta *metadata.Store, repoID, lang, repoRoot string, maxFilesPerSlice, maxIndexRunesPerSlice int) (content, path string, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	content, path, _, err = g.GenerateOverviewWithMeta(ctx, meta, lang, maxFilesPerSlice, maxIndexRunesPerSlice, OverviewGenerateOpts{RepoRoot: repoRoot, RepoID: repoID})
	if err != nil {
	...
	if strings.TrimSpace(content) != "" {
		func() {
			...
			if visual := BuildOverviewVisualSections(ctx, meta, repoID, lang, repoRoot, path); visual != "" {
				content += visual
			}
		}()
	}
