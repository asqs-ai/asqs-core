...
func diagnoseAlreadyReferencedButUnresolved(repoScanRoot, targetProjRel string, alreadyRefs []string, compileOutput string) alreadyReferencedDiag {
	...
	if out.PrivateFeedDetected {
		...
		out.Remediation = append(out.Remediation,
			"authenticate the private NuGet feed via ASQS config: for Azure DevOps set `general.git.azure_devops.token` + `general.sandbox.registries.azure_devops_nuget_feed_endpoints`; for any other HTTPS NuGet source (Artifactory, ProGet, BaGet, MyGet) add a `- {type: nuget, endpoint, username, password}` entry under `general.sandbox.registries.credentials` (the same unified list also covers Maven / npm with `type: maven` / `type: npm`). ASQS injects all paths via `VSS_NUGET_EXTERNAL_FEED_ENDPOINTS` on docker eval + bootstrap containers — no code change needed, just configuration. VSS_NUGET_ACCESSTOKEN / Credential Provider also work if you prefer to configure them at the container level.",
