...
func TestEvalFirstWaveMetricsForDB_errOrNilReturnsNil(t *testing.T) {
	if m := evalFirstWaveMetricsForDB(nil, nil, 0); m != nil {
		t.Fatalf("nil eval: want nil; got %+v", m)
	}
	if m := evalFirstWaveMetricsForDB(&evaluator.EvalWorkflowResult{Stable: true}, fmt.Errorf("x"), 100); m != nil {
...
func TestEvalFirstWaveMetricsForDB_mapsFields(t *testing.T) {
	...
	m := evalFirstWaveMetricsForDB(eval, nil, 1234)
	if m == nil {
...
func TestEvalFirstWaveMetricsForDB_tokensToStableRules(t *testing.T) {
	unstable := evalFirstWaveMetricsForDB(&evaluator.EvalWorkflowResult{Stable: false}, nil, 500)
	if unstable == nil || unstable.TokensToStable != nil {
		t.Fatalf("unstable run must omit tokens_to_stable: %+v", unstable)
	}
	untracked := evalFirstWaveMetricsForDB(&evaluator.EvalWorkflowResult{Stable: true}, nil, 0)
